---
name: detail-flow
description: Plan, generate, audit, revise, or deliver product detail pages and ecommerce long images from product photos, six-view references, packaging files, product concepts, screenshots, or style references. Use when the user says 生成新的详情页, asks for Chinese ecommerce详情页, unified 10-screen detail pages, product sales pages, digital-product long images, product-structure locking, visual master planning, staged image generation, stitched-preview QA, page-copy planning, SKU/white-background/5-image main-image planning that belongs to a detail-page project, and detail-page workflow optimization.
---
...
- PINGU products should use cool-cute, playful, giftable, daily-electronics styling, with the same complete style treatment as other authorized IPs: shallow blue / sky blue / baby blue as the recognizable base, controlled clash colors chosen from yellow, red/orange, pink, purple, cream/warm white, or black/white neutrals, contour paper-backed hero titles, sticker/tape rhythm, rounded props, desk/room/gift/shelf/tote/backpack/daily-carry scenes, and clear product-first ecommerce hierarchy. Do not use green, mint, teal-green, grass green, dark navy main fields, ice, snow, glaciers, igloos, winter-only worlds, empty catalog pedestals, or incorrect PINGU/Pingu product text.
...
- **Style direction**: IP identity, product category tone, target audience, style-fit reason, palette, analogous color extension, controlled clash-color / contrast-color plan, required two-or-more color families per scene, lighting, typography direction, text layout reference, card/icon language, prop density, layout diversity plan, and commercial-font/IP-text safety rule.
- **Visual content direction**: planned hero product treatment, scene-rendering route, required scenes, model-use scene, backpack portability scene, packaging/list treatment, props, product rendering style, product angles, product scale rhythm, foreground/background placement, background-color relationship, first-screen title placement, and what must not be hidden.
...
- Put hero composition, screen-01 product focus, model-use requirement, backpack portability requirement, screen-02 grid proof plan, product angles, scale/depth rhythm, background relationship, packaging/list treatment, and hidden-structure risks under `画面方向`.
...
For Chinese ecommerce digital-product pages, default to exactly 10 screens unless the user explicitly asks for another screen count or platform format. Do not expand to 11-13 screens by habit. A shorter 8-screen page is acceptable only when the user explicitly asks for 8 screens, a lighter sales page, SaaS/model pages, or a non-Chinese marketplace page.

The standard 10-screen blueprint must include these fixed modules:

- Screen 01: product-first hero. The product must be instantly eye-catching, large, clear, well lit, and arranged with strong visual taste. Scene, props, title, and package may support the product, but they must not steal focus. Product placement must use scale, diagonal tension, high/low bases, overlap, foreground/background depth, and clean negative space to create a beautiful ecommerce composition.
- Screen 02: fixed 2x3 six-card selling-point grid. Use exactly 6 buyer-facing cards unless the user explicitly requests fewer. Each card needs a number, short title, short explanation, and a visual proof.
- One parameter screen: include a clean parameter table or information sheet with verified parameters only. Unknown fields remain blank or `以实际参数为准`.
- One dedicated use/portability proof screen: include either a realistic model-use image or a backpack/travel-bag portability image. If the product category benefits from both and the page is not crowded, include both as separate screens inside the 10-page plan.
...
When screen 01 uses a paper-card title treatment for cute/IP pages, default to a contour paper backing rather than a full rectangular board: the off-white/light paper shape should tightly follow the combined outer contour of the main title text like a thick sticker outline, extending only a small margin beyond the letters, with subtle paper texture and soft shadow. It should feel like the paper wraps around and frames the title words. Do not make a large blocky paper sheet behind the entire title unless the user explicitly asks for a poster board.
...
For digital ecommerce pages, screen 02 is fixed as the product core selling-point overview. Use a grid/card layout similar to the user's reference: an exact 2x3 six-point grid unless the user explicitly requests fewer cards. Each card must include a visible number, short title, short explanation, and one supporting visual such as product crop, structure proof, icon, mini scene, phone/device relation, portability proof, or SKU/color proof. Do not turn screen 02 into a single hero poster, one large scene, or a repeated screen-01 visual. Use varied modules across the rest of the page: mechanism, structure proof, function experience, scenario, detail craft, compatibility, safety/trust, packaging/list, parameter table, comparison, and closing CTA.

Every ecommerce detail page blueprint must include these required visual modules unless the user explicitly removes them:

- **Hero product focus**: screen 01 must make the product immediately noticeable. The product should be the dominant visual subject, large, clear, well lit, aesthetically staged, and not crowded by props, models, packaging, or text.
- **Core selling-point grid**: screen 02 must summarize the product's main buyer-facing reasons in an exact 2x3 six-point grid, with short text and visual evidence in every card.
- **Parameter screen**: one screen must be a parameter table or parameter information sheet using verified values only; unknown values stay blank or `以实际参数为准`.
- **Model-use or backpack portability image**: at least one screen must show either realistic model/hand use or the product being placed into, partly inside, or beside a backpack/travel bag. The proof scene must communicate real use or portable storage while keeping product scale believable and key structures readable.
...
Typography must be license-safe. For commercial ecommerce images, plan titles, subtitles, labels, and cards with open-source or free-for-commercial-use font directions, or reserve editable text layers for later replacement with verified licensed fonts. Do not use, name, imitate, or rely on paid commercial fonts, proprietary brand fonts, trial fonts, pirated fonts, personal-use-only fonts, non-commercial-use-only fonts, or any font with unclear authorization. AI-rendered raster text is only a preview unless manually replaced or verified. Official IP wordmarks and legal text should come from supplied assets, not AI imitation.

Use diverse detail-page layouts. For the standard 10-screen ecommerce page, use at least 7 distinct layout archetypes. For 8-screen pages, use at least 5 distinct layout archetypes. Adjacent screens must not use the same visual grammar, text rhythm, product scale, and card structure at the same time. Borrow only layout language from user-supplied reference pages, such as large hero product, sticker callouts, multi-card overview, macro magnifier, diagonal product angle, hand operation, backpack portability, polaroid/photo frame, comic speech bubble, split color block, step sequence, package/list grid, parameter sheet, and SKU collection. Do not copy the reference page's exact text, fonts, official IP elements, character art, logo treatment, or unrelated product details.
...
- Screen 02 is a fixed core selling-point six-grid with exactly 6 numbered cards, short readable copy, and visual proof per card.
...
- IP tone, scene choice, title hierarchy, and typography fit the target audience without adding unsupported claims, fake legal text, commercial font names, trial/pirated/personal-use/non-commercial-use fonts, unclear font authorization, or AI-imitation official wordmarks.
...
- Chinese ecommerce detail page: every detail slice uses a vertical 9:16 ratio by default. If the platform keeps 790px width, use about 790 x 1404px per screen.
- Generator ratio fallback: vertical 9:16. Do not use extra-tall detail slices unless the user explicitly requests a long-scroll slice format.
...
2. Generate screen 02 second from the same product DNA and visual/IP style system. Screen 02 must be the fixed core selling-point grid: exact 2x3 six cards, with number badge, buyer-facing title, short explanation, and visual proof in each card.
...
- **Main images after detail pages**: generate a unified 5-image `1:1` main-image set unless the user explicitly asks for another count. Derive the 5 images from the approved 10-screen detail-page story, product DNA, and visual master. Do not create one main image per detail-page slice by default. Choose the five strongest square-selling themes: hero/product attraction, core function or performance, model-use or backpack portability, packaging/gift/list, and SKU/color or structure/detail proof. Preserve the slice's selling point, product angle, style, and copy hierarchy, but recompose each for square format instead of cropping, stretching, or squeezing the long slice. Keep the product prominent and readable in every square image.
...
- **SKU images**: generate only after the detail page and main-image set are complete. SKU images are `1:1` square, default 1400 x 1400 unless the platform requires otherwise. Decide SKU image count from confirmed product colors. If there are three confirmed colors, create three SKU images; if there is one confirmed color, create one SKU image. Use real colors/SKUs only.
- **SKU core structure**: SKU images are scene-based square information images, not simple white-background product cutouts. Keep the usual structure: top-left or left-side SKU/category title, color/SKU name, 5-9 buyer-facing selling points in pills/check rows/brush strips/cards, center-right or right-side hero product, optional packaging/accessory behind or beside the product when supplied, small color tag or collection mark near the lower edge, and any authorization/legal placeholder only when the user supplies it. The product must occupy about 45%-65% of the square, remain the largest readable subject, and use pedestals, product stands, packaging, foreground accessories, or front/back high-low placement to create depth.
- **SKU style continuity**: SKU scene style must be adjusted from the approved detail-page and main-image visual master. Detail pages, main images, and SKU images must share the same IP mood, product rendering route, lighting family, palette logic, title/card language, icon/sticker style, prop density, and typography safety. Do not invent an independent SKU visual system after the detail page is approved. Only convert the established visual system into a square SKU information layout.
- **SKU layout by category**: earphones use case + earbuds as the main subject, with earbuds close to the case and staggered by height, front/back depth, or floating angle; hair dryers use product + package + nozzle/cord/accessory with the product in front; power banks and hand warmers can show front/back/open states, screen or mirror proof, and handle/strap; fans, cables, stands, and accessories use the most recognizable product angle plus packaging or interface proof. Every SKU must preserve real structure, real color, same scale logic, and readable selling points.
...
Fail a result if it contains product redesign, wrong proportions, wrong port/button/screen/line/hinge position, missing real structure, extra parts, wrong SKU color, wrong package shape, invented parameters, fake legal/authorization text, unsupported IP slogans or bundle claims, PINGU rendered with dark navy as the main mood when the user wants light blue, green/mint/teal-green elements in a PINGU scene, PVC-style figures that dominate or imply unconfirmed accessories, commercial-font names, paid/trial/pirated/personal-use-only/non-commercial-use-only fonts, unclear font authorization, unreadable critical text, copied unrelated reference text, distorted or unreliable AI-rendered official wordmarks, PINGU/Pingu product text changed to incorrect words, contrast colors that overpower the product, a scene that uses only one color family, palette shifts that change the product/IP identity, repeated standalone-poster layouts, repeated adjacent layout grammar, insufficient layout diversity, or broken page continuity.
...
After final audit, save the complete approved set in a clear output folder. Include the approved master when used, all 10 final detail slices with sequential filenames, the early 2-slice preview, the full stitched long image, the unified 5-image `1:1` main-image set when requested, and the `1:1` SKU image set based on confirmed colors when requested. Exclude rejected or superseded variants unless the user asks for all iterations.
