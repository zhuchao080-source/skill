...
Default Chinese ecommerce detail-page slices use a vertical 9:16 ratio. If the platform keeps 790px width, use about 790 x 1404px per screen. Use a unified 10-screen structure for Chinese digital-product detail pages unless the user explicitly asks for another count; use 8 screens only when requested or when the product information is light.

Screen 01 must be product-first and visually attractive: the product should be large, clear, eye-catching, and placed with aesthetic scene composition rather than flat catalog centering.

Screen 02 must be a 2x3 six-selling-point card overview. Each card has one selling point, short title, 15-20 character explanation, icon/product detail/scene thumbnail, unified radius, spacing, shadow, and typography.

Every 10-screen detail page must include one parameter screen and at least one model-use or backpack/travel-bag portability screen. Unknown parameters stay blank or "以实际参数为准".
...
Use 1400 x 1400 square. Plan a unified 5-image main-image set by default: hero/product attraction, core function or performance, model-use or backpack portability, packaging/gift/list, and SKU/color display or structure/detail proof. Keep product scale, angle logic, and lighting consistent.

### SKU Images

Use real colors and packages only. SKU images are `1:1` square scene information images, not simple white-background cutouts. Generate them only after the detail page and main-image set are complete.

Keep SKU scene style derived from the approved detail-page and main-image visual master. Detail pages, main images, and SKU images must share the same IP mood, product rendering route, lighting family, palette logic, title/card language, icon/sticker style, prop density, and typography safety. Do not invent an independent SKU visual system; only convert the established detail-page style into a square SKU information layout.

Core SKU structure: top-left or left-side category/SKU title, color/SKU name, 5-9 short buyer-facing selling points, center-right or right-side corresponding product in true color and structure, optional supplied packaging/accessory behind or beside the product, small color tag or collection mark near the lower edge, and legal/authorization placeholders only when supplied. Product should occupy about 45%-65% of the square canvas and stay the largest readable subject.

Use pedestals, product stands, packaging, foreground accessories, overlap, height difference, front/back depth, and scale to guide the eye toward the product. Do not use drawn guide lines unless requested. Background and props support that SKU color and the approved detail-page palette but do not overpower the product. Include a collection SKU image only for real multi-color products.

Category layout guidance: earphones use case + earbuds as the main subject with earbuds near the case and staggered by height/depth/floating angle; hair dryers use product + packaging + nozzle/cord/accessory with product in front; power banks, hand warmers, and electronic scales may show front/back/open states when structurally confirmed; fans, cables, stands, and small accessories use the most recognizable product angle plus package/interface/connector proof.
